...
describe('db/csv', function () {
  describe('Ensure challenge data integrity', function () {
      ...
      describe(`for season ${season}:`, function () {
        ...
        it('should contain one entry (line) per contestant', function (done) {
          fs.readFile(this.filePath, function (error, challengeData) {
            fs.readFile(path.join(__dirname, `../../../db/csv/contestants/season-${season}.csv`), function (error, contestantData) {
              ...
              assert.strictEqual(challengeLines.length, contestantLines.length + 1, message);
        ...
        it('should contain a header with items delimited by a colon (:)', function (done) {
          fs.readFile(this.filePath, function (error, data) {
            ...
            assert.strictEqual(headers[0], 'Place', 'The first header should be "Place"');
            assert.strictEqual(headers[1], 'Contestant', 'The second header should be "Contestant"');
            ...
            headers.slice(2).forEach((headerItem) => {
              ...
              if (episodeHeader == episodeCounter) {
                ...
                assert.strictEqual(episodeHeader, episodeCounter, "episodes don't start with 1 or aren't sequential");
              }
            });
          ...
          fs.readFile(this.filePath, function (error, data) {
            ...
            toLines(data).splice(1).forEach((line) => {
              ...
              if (contestantPlace == placeCounter || contestantPlace == (placeCounter - 1)) {
                ...
                // we've got a problem here.
                assert.strictEqual(contestantPlace, placeCounter, "finishing place(s) are missing or aren't sequential'ish");
              }
            });
          ...
          fs.readFile(this.filePath, function (error, data) {
            toLines(data).splice(1).forEach(line => {
              ...
              challenges.forEach((challenge, index) => {
                ...
                assert.strictEqual(challengeParts.length, 2, `Invalid challenge parts for line: "${line}", challenge: ${challengeParts}`);
